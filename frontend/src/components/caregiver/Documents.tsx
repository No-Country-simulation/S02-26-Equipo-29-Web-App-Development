...
import {
  usePatientDocuments,
  useUploadPatientDocuments,
  useDeletePatientDocument,
} from "../../hooks/patient/usePatientDocuments";
...
export function Documents() {
  ...
  const { data: documentsData, isLoading: caregiverLoading } =
    useCaregiverDocuments();
  const { data: patientDataDoc, isLoading: patientLoading } =
    usePatientDocuments();
  ...
  const [patientDocuments, setPatientDocuments] = useState<UploadedDocument[]>(
    patientDataDoc || [],
  );
  ...
  if (caregiverLoading || patientLoading) {
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
        <div>
          ...
          activeDocuments?.find((doc) => doc.document_type === "dni_front") ? (
            <ImageDocument
              document={
                activeDocuments.find(
                  (doc) => doc.document_type === "dni_front",
                )!
              }
            ...
            <DocumentInput id="dni_front" handleFileUpload={handleFileUpload} />
          )}
        </div>
        <div>
          ...
          {activeDocuments?.find((doc) => doc.document_type === "dni_back") ? (
            ...
            <DocumentInput id="dni_back" handleFileUpload={handleFileUpload} />
          )}
        </div>
        <div>
...
const ImageDocument = ({
  ...
  handleDeleteDocFromServer,
}: {
  document: UploadedDocument;
  handleDeleteDocument: (id: string) => void;
  handleDeleteDocFromServer: (id: string) => void;
}) => {
  return (
    <div className="relative flex items-center justify-between rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3">
      ...
      <button
        onClick={
          document?.file_url
            ? () => handleDeleteDocFromServer(document.id)
